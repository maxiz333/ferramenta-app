...
function _fbFix(data){
  ...
  arr.forEach(function(item){
    ...
    if(!Array.isArray(item.items)){
      item.items=(typeof item.items==='object'&&item.items!==null)?Object.values(item.items).filter(function(x){return x!=null;}):[]; }
    item.items.forEach(function(it){
      if(it&&it.scaglioni&&!Array.isArray(it.scaglioni)){
        it.scaglioni=(typeof it.scaglioni==='object'&&it.scaglioni!==null)?Object.values(it.scaglioni).filter(function(x){return x!=null;}):[]; 
      }
    });
  });
  return arr;
...
function _fbPush(ref,data){
  if(!_fbReady||_fbSyncing)return;
  _fbDb.ref(ref).set(data).catch(function(e){
    console.warn('Firebase write error:',e&&e.message);
  });
}
...
function getNextOrdNum(){
  var num=parseInt(localStorage.getItem('cp4_ord_counter')||'0',10)+1;
...
function condividiWhatsApp(items,nomeCliente,totale,nota){
  ...
  if(!w){showToastGen('orange','Abilita i popup per aprire WhatsApp');}
}
...
function cartRemoveItem(cartId,idx){
  ...
  _takeSnapshot();
  var removedItem=(cart.items||[]).splice(idx,1)[0];
  ...
  btn.onclick=function(){(cart.items||[]).splice(idx,0,removedItem);saveCarrelli();renderCartTabs();t.remove();};
...
function _cartApplicaScaglione(it){
  ...
  if(!validi.length){
    if(it._prezzoBase){it.prezzoUnit=it._prezzoBase;delete it._prezzoBase;}
...
function getNextDDTNum(){
  var n = parseInt(localStorage.getItem(DDT_NUM_K) || '0', 10) + 1;
...
function stampaDDT(cartId){
  ...
  if(!ov){
    ...
    ov.style.cssText = 'position:fixed;top:0;right:0;bottom:0;left:0;inset:0;z-index:9999;background:#fff;overflow-y:auto;display:none;';
...
function _odApplicaScaglione(it){
  ...
  var qty=parseFloat(it.qty)||1;
  var sorted=it.scaglioni.slice().sort(function(a,b){return (b.qtaMin||0)-(a.qtaMin||0);});
  for(var i=0;i<sorted.length;i++){
    var sg=sorted[i];
...
function aggiungiCorrelato(i) {
  ...
  var ri = parseInt(selEl.value,10);
  if (!magazzino[i]) magazzino[i] = {};
...
function apriFotoImport() {
  ...
  // fi-url-input rimosso: elemento non presente nell'HTML
}
...
function gsGoArticolo(el){
  var raw=el?el.getAttribute('data-gi'):null;
...
function richiediNotifPermesso(){
...
function filterFatture(f){
  fatFiltro = f;
  ['tutte','emessa','ricevuta','scaduta'].forEach(function(x){
...
function renderFatture(){
  ...
  var daIncass = emesse.filter(function(f){return f.statoPag!=='pagata';}).reduce(function(s,f){return s+parseFloat(f.importo||0);},0);
  var daPagare = ricevute.filter(function(f){return f.statoPag!=='pagata';}).reduce(function(s,f){return s+parseFloat(f.importo||0);},0);
  if(statsEl) statsEl.innerHTML =
...
function salvaFattura(){
  ...
  if(_fatEditId){
    ...
    if(idx>=0) fatture[idx] = Object.assign(fatture[idx],{tipo:tipo,numero:numero,soggetto:soggetto,data:data,scadenza:scadenza,importo:importo,iva:iva,statoPag:statoPag,note:note});
  } else {
    fatture.unshift({id:'fat_'+Date.now(),tipo:tipo,numero:numero,soggetto:soggetto,data:data,scadenza:scadenza,importo:importo,iva:iva,statoPag:statoPag,note:note});
  }
...
function filterOrdFor(f){
  ordForFiltro = f;
  ['tutti','atteso','ricevuto','annullato'].forEach(function(x){
...
function renderFornitori(){
  ...
  filtered.sort(function(a,b){return (b.dataOrdine||'').localeCompare(a.dataOrdine||'');});
  if(!filtered.length){
...
setTimeout(richiediNotifPermesso, 1500);
loadEditorSettings();
applyEditorCSS();
...
(function(){
  try{
    ...
    }, function(err){console.error('FB ordini listener:',err&&err.message);});
    _fbDb.ref('carrelli').on('value',function(snap){
      ...
      setTimeout(function(){_fbSyncing=false;},500);
    }, function(err){console.error('FB carrelli listener:',err&&err.message);});
