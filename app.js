...
function cartDelta(cartId,idx,delta){
  ...
  cart.items[idx].qty=Math.max(0.5,Math.round(((parseFloat(cart.items[idx].qty)||0)+delta)*10)/10);
...
function cartSetQty(cartId,idx,val){
  var cart=carrelli.find(function(c){return c.id===cartId;});
  if(!cart||!cart.items[idx])return;
  cart.items[idx].qty=Math.max(0.5,parseFloat(val)||1);
  _cartApplicaScaglione(cart.items[idx]);
  saveCarrelli();renderCartTabs();
}
...
// --- RENDER CARRELLO ---------------------------------------
// ══════════════════════════════════════════════════════════════════════════
// NUOVE FUNZIONI — Tab Carrello chirurgico per iPhone
// ══════════════════════════════════════════════════════════════════════════

// ── cartToggleFnPanel ──────────────────────────────────────────────────────
// Apre/chiude il pannello funzione di un articolo (taglia/sconto/scaglioni/nota).
// Show on Click: se il pannello richiesto è già aperto, lo chiude.
// Un solo pannello alla volta per mantenere la card compatta.
// (State = stato dell'applicazione, qui _activePanel sull'item)
function cartToggleFnPanel(cartId, idx, panel){
  var cart = carrelli.find(function(c){ return c.id === cartId; });
  if(!cart || !cart.items[idx]) return;
  var it = cart.items[idx];
  // Toggle (inversione booleana): se già aperto chiude, altrimenti apre
  if(it._activePanel === panel){
    delete it._activePanel;
  } else {
    it._activePanel = panel;
    // Se il pannello è "scaglioni" e non ci sono righe, prepara una riga vuota
    if(panel === 'scaglioni'){
      if(!it.scaglioni) it.scaglioni = [];
      if(it.scaglioni.length === 0) it.scaglioni.push({qtaMin:'',sconto:'',prezzo:''});
      it._scaglioniAperti = true;
    }
  }
  // Non salva su Firebase: _activePanel è stato UI temporaneo (solo in memoria)
  renderCartTabs();
}

// ── cartToggleSpecs ────────────────────────────────────────────────────────
// Apre/chiude la sezione specifiche tecniche dell'articolo.
// (Collapse = sezione espandibile/collassabile, comune nelle interfacce mobile)
function cartToggleSpecs(cartId, idx){
  var cart = carrelli.find(function(c){ return c.id === cartId; });
  if(!cart || !cart.items[idx]) return;
  cart.items[idx]._specsOpen = !cart.items[idx]._specsOpen;
  renderCartTabs(); // ri-renderizza per mostrare le specifiche
}

// ── cartOpenColorPicker ────────────────────────────────────────────────────
// Apre un Popup (finestra sovrapposta leggera) per scegliere il colore
// dell'ordine fornitore. Il colore categorizza l'articolo per la futura
// Tab "Ordini Fornitore" (es. rosso=urgente, verde=normale, blu=preventivo).
// (Popup = div sovrapposto con z-index elevato, posizionato vicino al tasto)
function cartOpenColorPicker(cartId, idx){
  // Rimuove popup precedente se esistente (evita duplicati nel DOM)
  var existing = document.getElementById('cc-color-popup');
  ...
  // Colori predefiniti per categorizzazione ordini fornitore
  var colors = [
    {c:'#e53e3e', label:'Urgente'},
    {c:'#38a169', label:'Normale'},
    {c:'#3182ce', label:'Preventivo'},
    {c:'#805ad5', label:'Speciale'},
    {c:'#e2a000', label:'In attesa'},
    {c:'#718096', label:'Sospeso'},
  ];

  // Crea popup (Modal leggero = div posizionato fixed sullo schermo)
  var popup = document.createElement('div');
  popup.id = 'cc-color-popup';
  popup.className = 'cc-color-popup';

  // Stato corrente: se già "da ordinare" mostra opzione per rimuovere
  var h = '<div class="cc-color-popup-title">🛒 Categoria ordine</div>';
  h += '<div class="cc-color-grid">';
  colors.forEach(function(col){
    var isActive = it._ordColor === col.c;
    h += '<button class="cc-color-swatch' + (isActive ? ' cc-color-swatch--active' : '') + '"';
    h += ' style="background:' + col.c + '"';
    h += ' onclick="cartSetOrdColor(\'' + cartId + '\',' + idx + ',\'' + col.c + '\')"';
    h += ' title="' + col.label + '">';
    h += isActive ? '✓' : '';
    h += '</button>';
  });
  h += '</div>';
  if(it.daOrdinare || it._ordColor){
    h += '<button class="cc-color-remove" onclick="cartSetOrdColor(\'' + cartId + '\',' + idx + ',\'\')">';
    h += '✕ Rimuovi "da ordinare"</button>';
  }
  popup.innerHTML = h;

  // Chiude il popup cliccando fuori (EventListener sul backdrop)
  var backdrop = document.createElement('div');
  backdrop.id = 'cc-color-backdrop';
  backdrop.className = 'cc-color-backdrop';
...
// ── cartSetOrdColor ────────────────────────────────────────────────────────
// Salva il colore scelto sull'item e attiva "da ordinare" se c'è un colore.
function cartSetOrdColor(cartId, idx, color){
  var cart = carrelli.find(function(c){ return c.id === cartId; });
  if(!cart || !cart.items[idx]) return;
  var it = cart.items[idx];
  if(color){
  ...
  // Chiude il popup (rimuove dal DOM)
  var popup = document.getElementById('cc-color-popup');
  var backdrop = document.getElementById('cc-color-backdrop');
...
// ── openFotoModal ──────────────────────────────────────────────────────────
// Apre un Modal (finestra sovrapposta) per vedere la foto del prodotto ingrandita.
// Modal = overlay a schermo intero con z-index alto; si chiude toccando fuori.
// (z-index = proprietà CSS che controlla l'ordine di sovrapposizione degli elementi)
function openFotoModal(fotoSrc, descArt){
  // Rimuove modal precedente se esistente
  var existing = document.getElementById('cc-foto-modal');
  if(existing) existing.remove();

  var modal = document.createElement('div');
  modal.id = 'cc-foto-modal';
  modal.className = 'cc-foto-modal';
  // Chiude toccando il backdrop (EventListener sul click)
  modal.addEventListener('click', function(e){
    if(e.target === modal) modal.remove();
  });

  var h = '<div class="cc-foto-modal-box">';
  h += '<button class="cc-foto-modal-close" onclick="document.getElementById(\'cc-foto-modal\').remove()">✕</button>';
  h += '<img src="' + fotoSrc + '" alt="' + descArt + '" class="cc-foto-modal-img" loading="lazy">';
  if(descArt) h += '<div class="cc-foto-modal-caption">' + descArt + '</div>';
  h += '</div>';
  modal.innerHTML = h;
...
// ── cassaDoubleTap ─────────────────────────────────────────────────────────
// Sicurezza invio a cassa: richiede doppio tap ravvicinato (< 450ms).
// Al primo tap mostra "SICURO?" con animazione; al secondo esegue l'azione.
// (Timer = setTimeout, funzione che esegue dopo un ritardo in millisecondi)
// Questo previene invii accidentali mentre ci si muove in ferramenta.
var _cassaTapTimer  = null; // Timer per finestra doppio tap
var _cassaTapFirst  = {};   // Mappa cartId → stato primo tap

function cassaDoubleTap(cartId, action){
  var btnId = 'btn-conferma-' + cartId;
  var btn   = document.getElementById(btnId);

  if(_cassaTapFirst[cartId]){
    // ── Secondo tap: esegue l'azione ────────────────────────────────────
    clearTimeout(_cassaTapTimer);
    delete _cassaTapFirst[cartId];
    if(btn){
      btn.classList.remove('cc-btn-confirm--warn');
      btn.querySelector('span').textContent = action === 'aggiorna' ? 'AGGIORNA' : 'CONFERMA';
    }
    if(action === 'aggiorna') aggiornaOrdine(cartId);
    else                      inviaOrdine(cartId);
  } else {
    // ── Primo tap: mostra stato "SICURO?" ────────────────────────────────
    _cassaTapFirst[cartId] = true;
    if(btn){
      btn.classList.add('cc-btn-confirm--warn');
      btn.querySelector('span').textContent = 'SICURO?';
    }
    // Ripristina dopo 1500ms se non arriva il secondo tap
    _cassaTapTimer = setTimeout(function(){
      delete _cassaTapFirst[cartId];
      if(btn){
        btn.classList.remove('cc-btn-confirm--warn');
...
function renderCartTabs(){
  ...
  // ── TAB PILLS ────────────────────────────────────────────────────────────
  var tabsHtml = '<button class="cart-tab-pill new-cart-pill" onclick="newCart()">+ NUOVO</button>';
  carrelli.forEach(function(cart, ci){
    ...
    pill.className = 'cart-tab-pill' + (active ? ' active' : '');
    pill.dataset.tipo = tipo;
    var stateIcon = isInv ? '✅ ' : isMod ? '✏️ ' : '';
    pill.innerHTML = stateIcon + esc(cart.nome) +
      (n ? '<span style="background:rgba(0,0,0,.35);border-radius:8px;padding:1px 5px;font-size:9px;margin-left:4px;">' + n + '</span>' : '');
  ...
  // re-attach events (innerHTML strips them)
  var pills = bar.querySelectorAll('.cart-tab-pill:not(.new-cart-pill)');
  pills.forEach(function(pill, ci){
    pill.addEventListener('click',  function(){ switchCart(ci); });
    pill.addEventListener('dblclick', function(e){ e.stopPropagation(); rinominaCart(ci); });
  });
  ...
  if(!activeCartId || !carrelli.length){
    body.innerHTML =
      ...
      '<p>Premi <b style="color:var(--accent)">+ NUOVO</b> per iniziare</p>' +
      '</div>';
    return;
  }
  var cart = carrelli.find(function(c){ return c.id === activeCartId; });
  ...
  if(cart.stato === 'inviato' && cart.locked){
    var totInv = (cart.items||[]).reduce(function(s,it){ return s + parsePriceIT(it.prezzoUnit) * parseFloat(it.qty||0); }, 0);
    h += '<div style="background:#1a2a1a;border:1px solid #38a16944;border-radius:12px;padding:14px;margin-bottom:12px;">';
    h += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">';
    h += '<span style="font-size:22px;">✅</span>';
    h += '<div style="flex:1"><div style="font-size:14px;font-weight:800;color:#68d391;">Ordine inviato alla cassa</div>';
    h += '<div style="font-size:10px;color:#555;">' + esc(cart.nome) + '</div></div>';
    h += '<div style="font-size:18px;font-weight:900;color:var(--accent);">€ ' + totInv.toFixed(2) + '</div>';
    h += '</div>';
    (cart.items||[]).forEach(function(it){
      var sub = (parsePriceIT(it.prezzoUnit) * parseFloat(it.qty||0)).toFixed(2);
      h += '<div style="padding:5px 0;border-bottom:1px solid #1e2e1e;display:flex;justify-content:space-between;align-items:center;">';
      h += '<div style="flex:1;min-width:0;font-size:12px;color:#ccc;">' + esc(it.desc||'');
      if(it.codM) h += ' <span style="color:var(--accent);font-size:10px;">' + esc(it.codM) + '</span>';
      if(it.nota) h += '<div style="font-size:10px;color:#f6ad55;font-style:italic;">📝 ' + esc(it.nota) + '</div>';
      h += '</div>';
      h += '<div style="text-align:right;flex-shrink:0;font-size:12px;font-weight:700;color:var(--accent);">€' + sub + '</div></div>';
    });
    h += '<div style="display:flex;gap:8px;margin-top:14px;">';
    h += '<button onclick="cartUnlock(\'' + cart.id + '\')" style="flex:1;padding:10px;border-radius:8px;border:1px solid #805ad5;background:rgba(128,90,213,.1);color:#b794f4;font-size:13px;font-weight:700;cursor:pointer;">✏️ Sblocca e modifica</button>';
    h += '<button onclick="goTab(\'to\')" style="padding:10px 16px;border-radius:8px;border:none;background:var(--accent);color:#111;font-size:13px;font-weight:800;cursor:pointer;">📋 Ordini</button>';
    h += '</div>';
    h += '<div style="display:flex;gap:6px;margin-top:8px;">';
    h += '<button onclick="deleteCart(\'' + cart.id + '\')" style="flex:1;padding:8px;border-radius:8px;border:1px solid #333;background:transparent;color:#555;font-size:12px;cursor:pointer;">🗑️ Elimina carrello</button>';
    h += '</div></div>';
    body.innerHTML = h;
    return;
  }
  ...
  if(cart.stato === 'modifica'){
    h += '<div style="background:#1e1e1e;border:1px solid #805ad533;border-left:3px solid #805ad5;border-radius:10px;padding:10px 14px;margin-bottom:10px;display:flex;align-items:center;gap:10px;">';
    h += '<span style="font-size:20px;">✏️</span>';
    h += '<div style="flex:1;"><div style="font-size:13px;font-weight:800;color:#b794f4;">MODIFICA IN CORSO</div>';
    h += '<div style="font-size:10px;color:#666;">' + esc(cart.nome) + ' — modifica e premi Aggiorna</div></div>';
    h += '</div>';
  }

  // ── TASTI AZIONE (CERCA + NUOVO ARTICOLO) ─────────────────────────────────
  h += '<div id="cart-action-btns">';
  h += '<button class="pos-btn-cerca" onclick="openCodeNumpad()">';
  h += '<span class="pos-btn-icon">🔍</span>';
  h += '<span>CERCA PER CODICE</span>';
  h += '</button>';
  ...
  h += 'style="width:100%;padding:10px 14px;border:1px solid var(--pos-border);border-radius:10px;font-size:13px;background:#1e1e1e;color:var(--text);box-sizing:border-box;" ';
  ...
  if(!(cart.items||[]).length){
    h += '<div class="pos-empty-cart" style="padding:30px 20px;">';
    h += '<div class="pos-empty-icon">📦</div>';
    h += '<p>Cerca un articolo o premi <b style="color:var(--accent)">CERCA PER CODICE</b></p>';
    h += '</div>';
  } else {
    ...
    var totFinale = scontoGl ? tot * (1 - scontoGl/100) : tot;
    h += '<div class="cart-sticky-total">';
    h += '<div class="cart-sticky-bar">';
    h += '<div><span style="font-size:20px;font-weight:900;color:var(--accent);">€ ' + totFinale.toFixed(2) + '</span>';
    if(scontoGl) h += ' <span style="background:rgba(245,196,0,.15);color:var(--accent);padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;">-' + scontoGl + '%</span>';
    h += '</div>';
    h += '<div style="display:flex;align-items:center;gap:8px;">';
    h += '<span style="font-size:12px;color:var(--muted);font-weight:600;">' + (cart.items||[]).length + ' art.</span>';
    h += '<button onclick="openScontoOverlay()" style="padding:6px 10px;border-radius:8px;border:1px solid var(--accent)44;background:transparent;color:var(--accent);font-size:11px;font-weight:700;cursor:pointer;">% Sconto</button>';
    h += '</div></div></div>';

    // ── CARDS ARTICOLI — layout chirurgico per iPhone ──────────────────────
    // Struttura verticale per ogni articolo:
    //  [Foto popup] [Info: Nome, CodM, CodF, Q×UM, Prezzo sbarrato+finale]
    //  [Specifiche collassabili]
    //  [Barra funzioni: ✂️ % 📊 📝 🛒 | show-on-click]
    //  [Pannello dinamico: scampolo/sconto/scaglioni/nota — solo se attivo]
    (cart.items||[]).forEach(function(it, idx){
      ...
      // Codice magazzino a 7 cifre: padStart aggiunge zeri a sinistra se numerico
      var codM7 = it.codM ? (String(it.codM).match(/^\d+$/) ? String(it.codM).padStart(7,'0') : it.codM) : '';
      var codF  = it.codF || '';
      // Colore "Ordinare" salvato sull'item (per la futura Tab Ordini Fornitore)
      var ordColor = it._ordColor || '';
      // Prezzo listino originale (prima di sconti scampolo o scaglione)
      var prezzoListino = '';
      if(scagAtt && it._prezzoBase)         prezzoListino = (parsePriceIT(it._prezzoBase)*q).toFixed(2);
      else if(scOn && it._prezzoOriginale)  prezzoListino = (parsePriceIT(it._prezzoOriginale)*q).toFixed(2);

      // ── APERTURA CARD ─────────────────────────────────────────────────────
      h += '<div class="cc-card' + (it._checked ? ' cc-card--checked' : '') + '" id="cart-row-' + idx + '">';

      // ════════════════════════════════════════════════════════════════════
      // RIGA 1: [Foto interattiva] [Info prodotto compatta] [Cestino]
      // Flexbox (tecnica CSS che distribuisce gli elementi in una riga)
      // ════════════════════════════════════════════════════════════════════
      h += '<div class="cc-row1">';

      // ── FOTO INTERATTIVA ─────────────────────────────────────────────
      // Modal (finestra sovrapposta che mostra contenuto ingrandito) al click
      // EventListener (funzione che ascolta un evento, qui il click sulla foto)
      if(it.foto){
        h += '<div class="cc-thumb" onclick="openFotoModal(\'' + esc(it.foto) + '\',\'' + esc(it.desc||'') + '\')" title="Tocca per ingrandire">';
        h += '<img src="' + it.foto + '" alt="' + esc(it.desc||'') + '" loading="lazy">';
        h += '<div class="cc-thumb-zoom">🔍</div>';
        h += '</div>';
      } else {
        h += '<div class="cc-thumb cc-thumb--no-foto">📦</div>';
      }
      ...
      // Nome articolo
      h += '<div class="cc-name">' + esc(it.desc || '—') + '</div>';

      // Riga codici: codice magazzino (7 cifre) + codice fornitore
      if(codM7 || codF){
        h += '<div class="cc-codes">';
        if(codM7) h += '<span class="cc-code-m" title="Codice Magazzino">' + esc(codM7) + '</span>';
        if(codM7 && codF) h += '<span class="cc-code-sep">·</span>';
        if(codF)  h += '<span class="cc-code-f" title="Codice Fornitore">' + esc(codF) + '</span>';
        h += '</div>';
      }

      // Riga quantità × unità di misura (U.M.)
      h += '<div class="cc-qty-row">';
      // Qty stepper inline e compatto
      h += '<div class="cc-qty-wrap">';
      h += '<button class="cc-qty-btn" onclick="cartDelta(\'' + cart.id + '\',' + idx + ',-1)">−</button>';
      h += '<button class="cc-qty-val" onclick="openQtyNumpad(\'' + cart.id + '\',' + idx + ')">' + q + '</button>';
      h += '<button class="cc-qty-btn" onclick="cartDelta(\'' + cart.id + '\',' + idx + ',1)">＋</button>';
      h += '</div>';
      h += '<span class="cc-um">' + esc(it.unit||'pz') + '</span>';
      h += '</div>'; // fine cc-qty-row

      // ── PREZZI: listino sbarrato + finale + input unitario ───────────
      h += '<div class="cc-prices">';
      // Prezzo listino sbarrato (visible solo se c'è uno sconto attivo)
      if(prezzoListino){
        h += '<span class="cc-price-old">€' + prezzoListino + '</span>';
        // Badge risparmio
        var saving = (parseFloat(prezzoListino) - parseFloat(sub)).toFixed(2);
        if(parseFloat(saving) > 0) h += '<span class="cc-price-save">−€' + saving + '</span>';
      }
      // Prezzo finale in grassetto
      var subColor = scagAtt && it._prezzoBase ? 'var(--pos-orange2)' : (isFR ? '#f6ad55' : 'var(--accent)');
      h += '<span class="cc-price-final" style="color:' + subColor + '">€' + sub + '</span>';
      // Input prezzo unitario modificabile inline
      h += '<div class="cc-price-unit-wrap">';
      h += '<span class="cc-price-unit-lbl">€/u</span>';
      h += '<input class="cc-price-unit-inp pos-price-input" type="text" inputmode="decimal"';
      h += ' value="' + esc(it.prezzoUnit||'0') + '"';
      h += ' onchange="cartSetPrezzo(\'' + cart.id + '\',' + idx + ',this.value)"';
      h += ' onclick="this.select()" title="Modifica prezzo unitario">';
      h += '</div>';
      h += '</div>'; // fine cc-prices

      h += '</div>'; // fine cc-info

      // Cestino — a destra, verticalmente centrato
      h += '<button class="cc-del" onclick="cartRemoveItem(\'' + cart.id + '\',' + idx + ')" title="Rimuovi">🗑</button>';

      h += '</div>'; // fine cc-row1

      // ════════════════════════════════════════════════════════════════════
      // RIGA SPECIFICHE COLLASSABILI
      // Collapse (meccanismo di mostra/nascondi un blocco tramite click)
      // ════════════════════════════════════════════════════════════════════
      if(it.specs && it.specs.trim()){
        var specsOpen = it._specsOpen || false;
        h += '<div class="cc-specs-wrap">';
        h += '<button class="cc-specs-toggle" onclick="cartToggleSpecs(\'' + cart.id + '\',' + idx + ')">';
        h += '<span>📐 Specifiche</span>';
        h += '<span class="cc-specs-arrow">' + (specsOpen ? '▲' : '▼') + '</span>';
        h += '</button>';
        if(specsOpen){
          h += '<div class="cc-specs-body">' + esc(it.specs) + '</div>';
        }
        h += '</div>';
      }

      // ════════════════════════════════════════════════════════════════════
      // RIGA BARRA FUNZIONI — icone minimali, show-on-click
      // Show on Click (mostra contenuto solo quando si preme l'icona,
      //   non occupa spazio se non attivo)
      // ════════════════════════════════════════════════════════════════════
      h += '<div class="cc-fn-bar">';

      // Quale pannello è aperto? (solo uno alla volta per pulizia visiva)
      var activePanel = it._activePanel || '';

      // ── Icona TAGLIA / Scampolo / Fine Rotolo ────────────────────────
      var scIcon = isFR ? '🔚' : '✂️';
      h += '<button class="cc-fn-btn' + (scOn ? ' cc-fn-btn--on' : '') + (activePanel==='taglia' ? ' cc-fn-btn--open' : '') + '"';
      h += ' onclick="cartToggleFnPanel(\'' + cart.id + '\',' + idx + ',\'taglia\')"';
      h += ' title="Taglia / Scampolo">' + scIcon + '</button>';

      // ── Icona SCONTO % ────────────────────────────────────────────────
      var hasSconto = isSc || isFR;
      h += '<button class="cc-fn-btn' + (hasSconto ? ' cc-fn-btn--on' : '') + (activePanel==='sconto' ? ' cc-fn-btn--open' : '') + '"';
      h += ' onclick="cartToggleFnPanel(\'' + cart.id + '\',' + idx + ',\'sconto\')"';
      h += ' title="Sconto %">%</button>';

      // ── Icona SCAGLIONI ───────────────────────────────────────────────
      h += '<button class="cc-fn-btn' + (hasScag ? ' cc-fn-btn--on' : '') + (activePanel==='scaglioni' ? ' cc-fn-btn--open' : '') + '"';
      h += ' onclick="cartToggleFnPanel(\'' + cart.id + '\',' + idx + ',\'scaglioni\')"';
      h += ' title="Prezzi scaglionati">📊</button>';

      // ── Icona NOTE ────────────────────────────────────────────────────
      h += '<button class="cc-fn-btn' + (hasNota ? ' cc-fn-btn--on' : '') + (activePanel==='nota' ? ' cc-fn-btn--open' : '') + '"';
      h += ' onclick="cartToggleFnPanel(\'' + cart.id + '\',' + idx + ',\'nota\')"';
      h += ' title="Nota articolo">📝</button>';

      // ── Icona ORDINARE — colore distintivo, apre color-picker popup ──
      // Popup (finestra sovrapposta leggera per scelta rapida)
      // Il colore serve per categorizzare per la futura Tab Ordini Fornitore
      var ordBtnStyle = ordColor ? 'background:' + ordColor + ';color:#fff;border-color:' + ordColor + ';' : '';
      h += '<button class="cc-fn-btn cc-fn-btn--ordinare' + (isDaOrd ? ' cc-fn-btn--on' : '') + '"';
      h += ' style="' + ordBtnStyle + '"';
      h += ' onclick="cartOpenColorPicker(\'' + cart.id + '\',' + idx + ')"';
      h += ' title="Da ordinare — scegli colore fornitore">🛒</button>';

      h += '</div>'; // fine cc-fn-bar

      // ════════════════════════════════════════════════════════════════════
      // PANNELLO DINAMICO — appare solo se un'icona è attiva
      // Ogni pannello mostra i controlli di quel servizio specifico
      // ════════════════════════════════════════════════════════════════════

      // ── Pannello TAGLIA / Scampolo ─────────────────────────────────
      if(activePanel === 'taglia'){
        h += '<div class="cc-panel">';
        h += '<div class="cc-panel-label">';
        h += '<span>' + (isFR ? '🔚 Fine Rotolo' : '✂️ Scampolo') + '</span>';
        h += '<button class="cc-panel-cycle" onclick="cartCycleScampolo(\'' + cart.id + '\',' + idx + ')">';
        h += scOn ? (isFR ? 'Rimuovi' : 'Fine Rotolo →') : 'Attiva';
        h += '</button>';
        h += '</div>';
        if(scOn){
          h += '<div class="cc-panel-row">';
          h += '<label class="cc-panel-lbl">Sconto %</label>';
          h += '<input type="number" min="0" max="100" value="' + (it._scontoApplicato||30) + '"';
          h += ' class="cc-panel-inp" onchange="cartSetScontoScampolo(\'' + cart.id + '\',' + idx + ',this.value)">';
          h += '<span class="cc-panel-hint">Prezzo: €' + esc(it.prezzoUnit||'0') + '</span>';
          h += '</div>';
        }
        h += '</div>';
      }
      ...
      // ── Pannello SCONTO % ──────────────────────────────────────────
      if(activePanel === 'sconto' && !hasScag){
        h += '<div class="cc-panel">';
        h += '<div class="cc-panel-row">';
        h += '<label class="cc-panel-lbl">Sconto %</label>';
        h += '<input type="number" min="0" max="100" value="' + (it._scontoApplicato||'') + '"';
        h += ' class="cc-panel-inp" placeholder="0"';
        h += ' onchange="cartSetScontoScampolo(\'' + cart.id + '\',' + idx + ',this.value)">';
        h += '</div>';
        h += '</div>';
      }

      // ── Pannello SCAGLIONI ────────────────────────────────────────
      if(activePanel === 'scaglioni' || scagAp){
        var baseP    = it._prezzoBase || it.prezzoUnit || '0';
        var basePNum = parsePriceIT(baseP) || 0;
        h += '<div class="cc-panel cc-panel--scaglioni">';
        h += '<div style="font-size:10px;color:#63b3ed;margin-bottom:6px;">📊 Scaglioni — base €' + esc(baseP) + '</div>';
        (it.scaglioni||[]).forEach(function(sg, si){
          var pc = basePNum > 0 && sg.sconto ? (basePNum*(1-sg.sconto/100)).toFixed(2) : '';
          h += '<div class="cc-scag-row">';
          h += '<span class="cc-scag-lbl">da</span>';
          h += '<input type="number" min="1" value="' + (sg.qtaMin||'') + '" class="cc-scag-inp"';
          h += ' onchange="cartUpdScag(\'' + cart.id + '\',' + idx + ',' + si + ',\'qtaMin\',this.value)">';
          h += '<span class="cc-scag-lbl">pz —</span>';
          h += '<input type="number" min="0" max="100" value="' + (sg.sconto||'') + '" class="cc-scag-inp cc-scag-inp--pct"';
          h += ' onchange="cartUpdScag(\'' + cart.id + '\',' + idx + ',' + si + ',\'sconto\',this.value)">%';
          if(pc) h += '<span class="cc-scag-result">€' + pc + '</span>';
          h += '<button class="cc-scag-del" onclick="cartRmvScag(\'' + cart.id + '\',' + idx + ',' + si + ')">✕</button>';
          h += '</div>';
        });
        h += '<button class="cc-scag-add" onclick="cartAddScag(\'' + cart.id + '\',' + idx + ')">+ scaglione</button>';
        h += '</div>';
      }
      ...
      if(activePanel === 'nota'){
        h += '<div class="cc-panel">';
        h += '<input type="text" value="' + esc(it.nota||'') + '" placeholder="Nota articolo..." class="cc-nota-inp"';
        h += ' oninput="cartSetNota(\'' + cart.id + '\',' + idx + ',this.value)"';
        h += ' onblur="renderCartTabs()">';
        h += '</div>';
      } else if(hasNota) {
        // Nota preview quando pannello chiuso
        h += '<div class="cc-nota-preview">📝 ' + esc(it.nota) + '</div>';
      }
  ...
  h += 'oninput="cartSetNotaOrdine(\'' + cart.id + '\',this.value)">' + esc(cart.nota||'') + '</textarea>';
  h += '</div>';

  // ── FOOTER FISSO — 3 tasti operativi grandi ─────────────────────────────────
  // Calcola totale finale applicando eventuale sconto globale sul carrello
  ...
  h += '<div class="pos-footer-top">';
  h += '<div><span class="pos-footer-label">TOTALE ORDINE</span></div>';
  h += '<div class="pos-footer-total"><span class="pos-total-sym">€</span>' + tot2Fin.toFixed(2) + '</div>';
  h += '</div>';

  // ── TASTI FOOTER OPERATIVI ────────────────────────────────────────────────
  // Tre tasti grandi e ben separati, ognuno con funzione distinta
  h += '<div class="pos-footer-btns-big">';

  // Tasto 1: SVUOTA CARRELLO (rosso, sinistra)
  // Rimuove tutti gli articoli dal carrello attivo dopo conferma
  h += '<button class="pos-big-btn pos-big-btn-danger" ';
  h += 'onclick="svuotaCarrello(\'' + cart.id + '\')">';
  h += '🗑️<span>SVUOTA</span></button>';

  // Tasto 2: RIEPILOGO ORDINE (arancione, centro)
  // Apre overlay con checklist spuntabile di tutti gli articoli
  h += '<button class="pos-big-btn pos-big-btn-orange" ';
  h += 'onclick="openRiepilogoOrdine(\'' + cart.id + '\')">';
  h += '👀<span>RIEPILOGO</span></button>';

  // ── Tasto CONFERMA / AGGIORNA — sicurezza doppio tap ───────────────────
  // Double Tap (doppio tocco): richiede due tap ravvicinati (<400ms) per evitare
  // invii accidentali mentre ci si muove in negozio.
  // Un primo tap mostra il feedback "SICURO?" con animazione, il secondo conferma.
  if(cart.stato === 'modifica'){
    h += '<button class="pos-big-btn pos-big-btn-confirm" ';
    h += 'id="btn-conferma-' + cart.id + '" ';
    h += 'onclick="cassaDoubleTap(\'' + cart.id + '\',\'aggiorna\')">';
    h += '✏️<span>AGGIORNA</span></button>';
  } else {
    h += '<button class="pos-big-btn pos-big-btn-confirm" ';
    h += 'id="btn-conferma-' + cart.id + '" ';
    h += (!(cart.items||[]).length ? 'disabled ' : '');
    h += 'onclick="cassaDoubleTap(\'' + cart.id + '\',\'invia\')">';
    h += '🛍️<span>CONFERMA</span></button>';
  }
  h += '</div>';

  // ── TASTI SECONDARI (DDT + Cestino) ──────────────────────────────────────
  h += '<div class="pos-footer-btns">';
  h += '<button class="pos-footer-sec" onclick="stampaDDT(\'' + cart.id + '\')">';
  h += '<span class="pos-sec-icon">🖨️</span>DDT</button>';
  h += '<button class="pos-footer-sec danger" onclick="deleteCart(\'' + cart.id + '\')">';
  h += '<span class="pos-sec-icon">🗑️</span>Cestino</button>';
  h += '</div>';
  h += '</div>';
  body.innerHTML = h;
}
