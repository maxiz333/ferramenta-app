...
function renderCartTabs(){
  ...
  if(!(cart.items||[]).length){
    ...
    (cart.items||[]).forEach(function(it, idx){
      ...
      // Codice a 7 cifre: pad con zeri se numerico
      var codM7 = it.codM ? (String(it.codM).match(/^\d+$/) ? String(it.codM).padStart(7,'0') : it.codM) : '';
      var codF  = it.codF || '';

      h += '<div class="pos-cart-card ' + (it._checked ? 'pos-cart-card-checked' : '') + '" id="cart-row-' + idx + '">';

      // ── TOP ROW: thumb + info + prezzo + cestino ─────────────────────
      h += '<div class="pos-card-top">';

      // Thumb (foto o emoji)
      h += '<div class="pos-card-thumb">';
      if(it.foto) h += '<img src="' + it.foto + '" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">';
      else        h += '📦';
      h += '</div>';

      // Info: nome + codici
      h += '<div class="pos-card-info">';
      h += '<div class="pos-card-name">' + esc(it.desc || '—') + '</div>';
      if(codM7 || codF){
        h += '<div class="pos-card-code">';
        if(codM7) h += '<span style="color:var(--accent);">' + esc(codM7) + '</span>';
        if(codM7 && codF) h += ' · ';
        if(codF)  h += '<span style="color:#fc8181;">' + esc(codF) + '</span>';
        h += '</div>';
      }
      h += '</div>';

      // Prezzo modificabile + subtotale
      h += '<div style="text-align:right;flex-shrink:0;">';
      // Subtotale (calcolato, non editabile)
      var subColor = scagAtt && it._prezzoBase ? 'var(--pos-orange2)' : (isFR?'#f6ad55':'var(--accent)');
      h += '<div class="pos-card-price" style="color:' + subColor + ';">€' + sub + '</div>';
      if(scagAtt && it._prezzoBase)
        h += '<div class="pos-card-old-price">€' + (parsePriceIT(it._prezzoBase)*q).toFixed(2) + '</div>';
      if((isSc||isFR) && it._prezzoOriginale)
        h += '<div class="pos-card-old-price">€' + (parsePriceIT(it._prezzoOriginale)*q).toFixed(2) + '</div>';
      // Input prezzo unitario — modificabile inline
      h += '<div style="display:flex;align-items:center;gap:3px;justify-content:flex-end;margin-top:3px;">';
      h += '<span style="font-size:10px;color:var(--pos-muted);">€</span>';
      h += '<input class="pos-price-input" type="text" inputmode="decimal" ';
      h += 'value="' + esc(it.prezzoUnit||'0') + '" ';
      h += 'onchange="cartSetPrezzo(\'' + cart.id + '\',' + idx + ',this.value)" ';
      h += 'onclick="this.select()" ';
      h += 'title="Modifica prezzo unitario">';
      h += '<span style="font-size:10px;color:var(--pos-muted);">× ' + q + ' ' + (it.unit||'pz') + '</span>';
      h += '</div>';
      h += '</div>';

      // Cestino
      h += '<button class="pos-card-del" onclick="cartRemoveItem(\'' + cart.id + '\',' + idx + ')">🗑️</button>';

      h += '</div>'; // fine pos-card-top

      // ── Nota preview inline ────────────────────────────────────────────
      if(hasNota){
        h += '<div class="pos-card-nota-preview">📝 ' + esc(it.nota) + '</div>';
      }

      // ── ACTIONS ROW: qty + 5 tasti ────────────────────────────────────
      h += '<div class="pos-card-actions">';

      // Qty stepper
      h += '<div class="pos-qty-wrap">';
      h += '<button class="pos-qty-btn" onclick="cartDelta(\'' + cart.id + '\',' + idx + ',-1)">−</button>';
      h += '<input class="pos-qty-val" type="text" value="' + q + '" ';
      h += 'onclick="openQtyNumpad(\'' + cart.id + '\',' + idx + ')" readonly>';
      h += '<button class="pos-qty-btn" onclick="cartDelta(\'' + cart.id + '\',' + idx + ',1)">＋</button>';
      h += '</div>';

      // Tasto TAGLIA (scampolo/fine rotolo)
      var scOn  = isSc || isFR;
      var scLbl = isFR ? 'Fin.Rot.' : 'Taglia';
      h += '<button class="pos-act-btn ' + (scOn ? 'has-value' : '') + '" onclick="cartCycleScampolo(\'' + cart.id + '\',' + idx + ')">';
      h += '<span class="pos-act-icon">✂️</span>' + scLbl + '</button>';

      // Tasto SCONTO %
      var hasScag = scagAp || scagAtt;
      h += '<button class="pos-act-btn ' + (hasScag ? 'has-value' : '') + '" onclick="cartToggleScaglioni(\'' + cart.id + '\',' + idx + ')">';
      h += '<span class="pos-act-icon">%</span>Sconto</button>';

      // Tasto IMBALLO (da ordinare)
      h += '<button class="pos-act-btn ' + (isDaOrd ? 'has-value' : '') + '" onclick="cartToggleDaOrdinare(\'' + cart.id + '\',' + idx + ')">';
      h += '<span class="pos-act-icon">📦</span>Imballo</button>';

      // Tasto NOTE
      h += '<button class="pos-act-btn ' + (hasNota ? 'has-value' : '') + '" onclick="cartToggleNotaInline(\'' + cart.id + '\',' + idx + ')">';
      h += '<span class="pos-act-icon">📝</span>Note</button>';

      h += '</div>'; // fine pos-card-actions

      // ── Nota inline nascosta ────────────────────────────────────────────
      h += '<div id="cart-nota-' + idx + '" style="display:none;padding:0 10px 8px;">';
      h += '<input type="text" value="' + esc(it.nota||'') + '" placeholder="Nota articolo..." ';
      h += 'style="width:100%;padding:7px 10px;border:1px solid #f6ad5544;border-radius:8px;background:#2a1800;color:#f6ad55;font-size:12px;font-weight:600;outline:none;box-sizing:border-box;" ';
      h += 'oninput="cartSetNota(\'' + cart.id + '\',' + idx + ',this.value)" ';
      h += 'onblur="cartHideNota(' + idx + ')">';
      h += '</div>';

      // ── Badge attivi ────────────────────────────────────────────────────
      var badges = '';
      if(isDaOrd)   badges += '<span style="color:#fc8181;font-size:9px;font-weight:700;background:rgba(229,62,62,.1);padding:2px 6px;border-radius:4px;">🚚 DA ORDINARE</span> ';
      if(scagAtt)   badges += '<span style="color:#63b3ed;font-size:9px;font-weight:700;background:rgba(49,130,206,.1);padding:2px 6px;border-radius:4px;">📊 -' + (scagAtt.sconto||0) + '% da ' + scagAtt.qtaMin + 'pz</span> ';
      if(scOn && it._scontoApplicato) badges += '<span style="color:var(--accent);font-size:9px;font-weight:700;background:rgba(245,196,0,.1);padding:2px 6px;border-radius:4px;">' + (isFR?'🔚':'✂️') + ' -' + it._scontoApplicato + '%</span> ';
      if(badges) h += '<div style="padding:0 10px 8px;display:flex;flex-wrap:wrap;gap:4px;">' + badges + '</div>';

      // ── Scaglioni espandibili ────────────────────────────────────────────
      if(scagAp){
        var baseP = it._prezzoBase || it.prezzoUnit || '0';
        var basePNum = parsePriceIT(baseP) || 0;
        h += '<div style="margin:0 10px 8px;padding:8px;background:#0d1420;border:1px solid #3182ce22;border-radius:8px;">';
        h += '<div style="font-size:10px;color:#63b3ed;margin-bottom:4px;">📊 Scaglioni — base €' + esc(baseP) + '</div>';
        (it.scaglioni||[]).forEach(function(sg, si){
          ...
          h += '<span style="font-size:9px;color:#555;">da</span>';
          h += '<input type="number" min="1" value="' + (sg.qtaMin||'') + '" style="width:38px;padding:2px;border:1px solid #1a2a3a;border-radius:4px;background:#111;color:#e8e8e8;font-size:10px;text-align:center;" onchange="cartUpdScag(\'' + cart.id + '\',' + idx + ',' + si + ',\'qtaMin\',this.value)">';
          h += '<span style="font-size:9px;color:#555;">pz —</span>';
          h += '<input type="number" min="0" max="100" value="' + (sg.sconto||'') + '" style="width:32px;padding:2px;border:1px solid #1a2a3a;border-radius:4px;background:#111;color:#68d391;font-size:10px;text-align:center;" onchange="cartUpdScag(\'' + cart.id + '\',' + idx + ',' + si + ',\'sconto\',this.value)">%';
          if(pc) h += '<span style="font-size:10px;color:var(--accent);font-weight:700;">€' + pc + '</span>';
          h += '<button onclick="cartRmvScag(\'' + cart.id + '\',' + idx + ',' + si + ')" style="width:16px;height:16px;border:none;background:transparent;color:#555;font-size:10px;cursor:pointer;margin-left:auto;">✕</button>';
          h += '</div>';
        });
        h += '<button onclick="cartAddScag(\'' + cart.id + '\',' + idx + ')" style="padding:3px 8px;border-radius:4px;border:1px dashed #3182ce33;background:transparent;color:#3182ce;font-size:9px;cursor:pointer;margin-top:2px;">+ scaglione</button>';
        h += '</div>';
      }

      h += '</div>'; // fine pos-cart-card
    });
  } // fine items.length
  ...
  h += '<button class="pos-footer-sec pos-footer-riepilogo" onclick="openRiepilogoOrdine(\'' + cart.id + '\')">';
  h += '<span class="pos-sec-icon">✅</span>RIEPILOGO</button>';
  h += '<button class="pos-footer-sec danger" onclick="deleteCart(\'' + cart.id + '\')">';
  h += '<span class="pos-sec-icon">🗑️</span>Cestino</button>';
  if(cart.stato === 'modifica'){
    h += '<button class="pos-btn-cassa" onclick="aggiornaOrdine(\'' + cart.id + '\')" style="background:linear-gradient(135deg,#5a2d8a,#805ad5)!important;">';
    h += '✏️ AGGIORNA ORDINE</button>';
  } else {
    h += '<button class="pos-btn-cassa" onclick="inviaOrdine(\'' + cart.id + '\')" ' + (!(cart.items||[]).length ? 'disabled' : '') + '>';
    h += '🛒 INVIA A CASSA</button>';
  }
  h += '</div></div>';
...
function renderOrdini(){
  ...
  gruppiOrd.forEach(function(dk){
    h+='<div style="font-size:10px;font-weight:800;color:#555;text-transform:uppercase;letter-spacing:1px;padding:14px 4px 6px;">'+esc(dk)+'</div>';
    ...
    gruppi[dk].forEach(function(ord){
      ...
      // -- CARD ORDINE --
      h+='<div style="background:#1c1c1c;border-radius:12px;margin-bottom:8px;overflow:hidden;border-left:4px solid '+sc+';">';

      // Header: nome + stato + totale
      h+='<div style="padding:12px 14px 8px;display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">';
      h+='<div style="flex:1;min-width:0;">';
      h+='<div style="font-size:16px;font-weight:800;color:var(--text);">'+esc(ord.nomeCliente||'-')+'</div>';
      h+='<div style="font-size:10px;color:#555;margin-top:2px;">'+(ord.numero?'#'+ord.numero+' - ':'')+esc(ord.data||'')+(ord.ora?' '+ord.ora:'')+'</div>';
      h+='</div>';
      h+='<div style="text-align:right;flex-shrink:0;">';
      h+='<div style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:800;background:'+sc+'20;color:'+sc+';border:1px solid '+sc+'44;">'+(SI[ost]||'')+' '+SL[ost]+'</div>';
      h+='<div style="font-size:20px;font-weight:900;color:var(--accent);margin-top:4px;">-'+tot.toFixed(2)+'</div>';
      h+='</div></div>';

      // Nota ordine
      if(ord.nota){
        h+='<div style="margin:0 12px 8px;padding:6px 10px;background:#2a1800;border-left:3px solid #f6ad55;border-radius:4px;font-size:11px;color:#f6ad55;font-weight:600;">- '+esc(ord.nota)+'</div>';
      }
      ...
      (ord.items||[]).forEach(function(it){
        ...
        var q=parseFloat(it.qty||0);
        var sub=(pu*q).toFixed(2);
        h+='<div style="display:flex;align-items:baseline;gap:6px;padding:4px 0;border-bottom:1px solid #252525;">';
        h+='<span style="background:#252525;color:var(--accent);font-size:10px;font-weight:800;padding:2px 6px;border-radius:4px;flex-shrink:0;">'+q+'</span>';
        h+='<span style="font-size:14px;font-weight:700;color:#e8e8e8;flex:1;">'+esc(it.desc||'')+'</span>';
        h+='<span style="font-size:12px;font-weight:800;color:var(--accent);flex-shrink:0;">-'+sub+'</span>';
        h+='</div>';
        // Codici
        var metaOrd='';
        if(it.codM)metaOrd+='<span style="color:var(--accent);font-weight:700;">'+esc(it.codM)+'</span>';
        if(it.codF)metaOrd+=(metaOrd?' &nbsp;&nbsp; ':'')+'<span style="color:#fc8181;font-weight:700;">'+esc(it.codF)+'</span>';
        metaOrd+=(metaOrd?' &nbsp;&nbsp; ':'')+'<span style="color:#888;font-weight:700;">-'+pu.toFixed(2)+' / '+(it.unit||'pz')+'</span>';
        h+='<div style="font-size:11px;padding:3px 0 2px 2px;">'+metaOrd+'</div>';
        // Nota articolo
        if(it.nota){
          h+='<div style="margin:2px 0 4px;padding:3px 8px;background:#2a1800;border-left:3px solid #f6ad55;border-radius:3px;font-size:10px;color:#f6ad55;font-weight:600;">- '+esc(it.nota)+'</div>';
        }
        // Badge
        if(it.daOrdinare)h+='<span style="font-size:8px;color:#fc8181;font-weight:800;">- DA ORDINARE</span> ';
      });
      h+='</div>';

      // -- AZIONI --
      h+='<div style="display:flex;gap:6px;padding:8px 12px;background:#161616;align-items:center;flex-wrap:wrap;">';
      // Azione principale
      if(ost!=='completato'){
        h+='<button onclick="setStatoOrdine('+gi+',\'completato\')" style="padding:8px 18px;border-radius:10px;border:none;background:#38a169;color:#fff;font-size:13px;font-weight:800;cursor:pointer;">- Fatto</button>';
      } else {
        h+='<button onclick="setStatoOrdine('+gi+',\'nuovo\')" style="padding:6px 14px;border-radius:8px;border:1px solid #f5c40044;background:transparent;color:#f5c400;font-size:11px;font-weight:700;cursor:pointer;">- Riapri</button>';
      }
      // Modifica + Cassa
      h+='<button onclick="modificaOrdineDaTab('+gi+')" style="padding:6px 12px;border-radius:8px;border:1px solid #805ad533;background:transparent;color:#b794f4;font-size:11px;font-weight:700;cursor:pointer;">-- Modifica</button>';
      h+='<button onclick="openCassa('+gi+')" style="padding:6px 12px;border-radius:8px;border:1px solid #f5c40033;background:transparent;color:var(--accent);font-size:11px;cursor:pointer;">- Cassa</button>';
      // Stampa + stati secondari
      h+='<button onclick="stampaRicevuta(ordini['+gi+'].items,ordini['+gi+'].nomeCliente,ordini['+gi+'].totale,ordini['+gi+'].nota)" style="padding:4px 8px;border-radius:6px;border:1px solid #333;background:transparent;color:#666;font-size:10px;cursor:pointer;">-</button>';
      if(ost!=='lavorazione')h+='<button onclick="setStatoOrdine('+gi+',\'lavorazione\')" style="padding:4px 8px;border-radius:6px;border:1px solid #333;background:transparent;color:#666;font-size:10px;cursor:pointer;">-</button>';
      if(ost!=='pronto')h+='<button onclick="setStatoOrdine('+gi+',\'pronto\')" style="padding:4px 8px;border-radius:6px;border:1px solid #333;background:transparent;color:#666;font-size:10px;cursor:pointer;">-</button>';
      // Elimina (a destra)
      h+='<button onclick="deleteOrdine('+gi+')" style="margin-left:auto;padding:4px 8px;border-radius:6px;border:1px solid #e53e3e22;background:transparent;color:#e53e3e66;font-size:10px;cursor:pointer;">-</button>';
      h+='</div>';
...
// ═══════════════════════════════════════════════════════════════════════════════
//  TAB CARTELLINI (t1) — caricamento lazy, page-size ridotto, zero loop
//
//  BUG FIXES rispetto alle versioni precedenti:
//  1. goTabDirect usa window.goTabDirect per catturare la funzione di database.js
//     PRIMA che il function hoisting la sovrascriva — evita loop infinito
//  2. renderTable usa window.renderTable con lo stesso pattern
//  3. _renderTable_orig catturato inline (non in DOMContentLoaded) per
//     garantire che venga assegnato subito all'esecuzione dello script
//  4. Firebase SDK duplicato rimosso da index.html (da fare separatamente)
// ═══════════════════════════════════════════════════════════════════════════════

// ── 1. Riduce _tablePageSize subito ─────────────────────────────────────────
(function(){
...
// ── 2. Cattura la goTabDirect ORIGINALE (da database.js) tramite window
//       PRIMA che il function hoisting sovrascriva il binding locale
//       La soluzione: NON usare function declaration, usare var + function expression
// ────────────────────────────────────────────────────────────────────────────
var _goTabDirect_orig = window.goTabDirect || null;

// Sovrascrive goTabDirect su window (unico modo per battere il hoisting)
window.goTabDirect = function(id){
  // Per t1: lazy load se ci sono molti articoli
  if(id === 't1' && rows && rows.length > 200){
    // Attiva visivamente la tab
    ...
    // Evidenzia bottone tab bar (può essere tbb-t1 o tbb cartellini)
    var tbb = document.getElementById('tbb-t1');
    if(tbb) tbb.classList.add('active');
    // Chiama utilities non-render di goTabDirect
    if(typeof loadMagazzinoFB === 'function') loadMagazzinoFB();
    if(typeof _updateBackBtn === 'function') _updateBackBtn(id);
    if(typeof closeAltroMenu === 'function') closeAltroMenu();
    if(window.scrollTo) window.scrollTo(0, 0);
    // Mostra banner lazy (se ricerca vuota)
    var si = document.getElementById('si');
    var hasQuery = si && si.value.trim().length >= 2;
    if(!hasQuery) _showCartelliniLazy();
    return;
  }
  // Tutte le altre tab: chiama la funzione originale di database.js
  if(_goTabDirect_orig) _goTabDirect_orig(id);
};

// ── 3. Cattura renderTable ORIGINALE (da database.js) tramite window ────────
var _renderTable_orig = window.renderTable || null;

// Sovrascrive renderTable: blocca le chiamate quando t1 non è visibile
window.renderTable = function(){
  var t1 = document.getElementById('t1');
  if(t1 && !t1.classList.contains('active')){
    // t1 non visibile — aggiorna solo le stats senza DOM pesante
    if(typeof updateStats === 'function') updateStats();
    return;
  }
  // t1 attiva: renderizza normalmente
  if(_renderTable_orig) _renderTable_orig();
};

// ── 4. Banner lazy per tab Cartellini ───────────────────────────────────────
function _showCartelliniLazy(){
  var tb = document.getElementById('tb');
  if(!tb) return;
  // Se c'è già roba nel tbody (da un render precedente), non sovrascrivere
  if(tb.children.length && !tb.querySelector('#_cart_lazy_banner')) return;

  var cap = (typeof _tablePageSize !== 'undefined') ? _tablePageSize : 50;
  var active = rows.filter(function(r, i){ return r && !removed.has(String(i)); }).length;

  // Se pochi articoli, carica direttamente
  if(active <= cap){
    if(_renderTable_orig) _renderTable_orig();
  ...
  banner.innerHTML =
    ...
    '<div style="font-size:14px;font-weight:700;color:var(--accent);margin-bottom:6px;">' +
      rows.length.toLocaleString('it-IT') + ' articoli nel database' +
    '</div>' +
    '<div style="font-size:12px;color:var(--muted);margin-bottom:16px;">' +
      'Usa la barra di ricerca 🔍 per trovare articoli specifici,<br>' +
      'oppure carica i primi ' + cap + ' per sfogliare.' +
    '</div>' +
    '<button onclick="if(window._renderTable_orig)window._renderTable_orig();if(typeof genTags===\'function\')genTags();" ' +
    'style="padding:10px 24px;border-radius:10px;border:none;background:var(--accent);color:#111;font-size:13px;font-weight:800;cursor:pointer;touch-action:manipulation;">' +
    '📋 Carica i primi ' + cap + ' articoli</button>' +
    '</td>';
