...
function renderCartTabs(){
  ...
  if(!(cart.items||[]).length){
    ...
    (cart.items||[]).forEach(function(it, idx){
      ...
      // ── RIGA PRINCIPALE: [foto] [info] [qty+UM] [prezzo] ─────────────────
      h += '<div class="ct-row">';
      ...
      // Qty stepper + U.M. select dinamico
      h += '<div class="ct-qty-block">';
      h += '<div class="ct-qty">';
      ...
      h += '<button class="ct-icon-btn' + (scOn ? ' ct-icon-btn--on' : '') +
           (isTuttoRotolo ? ' ct-icon-btn--rotolo' : '') + '" ' +
           'onclick="ctForbiciClick(\'' + cart.id + '\',' + idx + ',event)" ' +
           'ondblclick="ctTuttoRotolo(\'' + cart.id + '\',' + idx + ')" ' +
           'title="Click: scampolo/rotolo | DblClick: TUTTO IL ROTOLO">';
      h += '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' +
           '<circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/>' +
           '<line x1="20" y1="4" x2="8.12" y2="15.88"/>' +
           '<line x1="14.47" y1="14.48" x2="20" y2="20"/>' +
           '<line x1="8.12" y1="8.12" x2="12" y2="12"/></svg>';
      h += (isTuttoRotolo ? '<span>ROTOLO</span>' : '') + '</button>';
...
function ctForbiciClick(cartId, idx, evt){
  // event.detail === 2 significa che questo click fa parte di un dblclick
  // In tal caso lo ignoriamo (sarà gestito da ondblclick)
  if(evt && evt.detail >= 2) return;
  // Avvia timer: se dopo 250ms non è arrivato il secondo click, esegue
...
// ctTuttoRotolo: DOPPIO CLICK sulle forbici — attiva "Tutto il Rotolo"
// ── Logica ──────────────────────────────────────────────────────────────────
// Il dblclick è un evento distinto da onclick — il browser lo emette
// dopo due click ravvicinati. Qui cancelliamo il timer del singolo click
// per evitare che scatti anche il ciclo scampolo.
// Effetti: bordo rosso sulla card, nota automatica "ROTOLO INTERO",
// flag it._tuttoRotolo = true che viene visualizzato nell'icon-bar.
function ctTuttoRotolo(cartId, idx){
  // Annulla il singolo click pendente (timer)
  clearTimeout(_ctForbiciTimers[idx]);
  ...
  if(it._tuttoRotolo){
    // Toggle: secondo dblclick disattiva la modalità
    it._tuttoRotolo = false;
    // Rimuove la nota automatica se era quella inserita da noi
    ...
    // Attiva "Tutto il Rotolo":
    it._tuttoRotolo = true;
    // Imposta nota automatica (sarà visibile nel riepilogo)
    it.nota        = 'ROTOLO INTERO';
    // Usa la logica fineRotolo esistente (sconto 0 = prezzo pieno rotolo)
    it.scampolo    = false;
    it.fineRotolo  = true;
    it._scontoTipo = 'rotolo';
    if(!it._prezzoOriginale) it._prezzoOriginale = it.prezzoUnit;
    it._scontoApplicato = 0; // nessuno sconto: prezzo pieno del rotolo intero
    // Non chiama _applicaScontoScampolo perché sconto = 0
  }
  saveCarrelli();
  renderCartTabs();
}
