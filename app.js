...
function renderCartTabs(){
  ...
  if(!(cart.items||[]).length){
    ...
    // ══ CARDS ARTICOLI — layout ULTRA-COMPACT per iPhone a una mano ══════
    // Struttura: [thumb?] [nome+codici] | [PREZZO TRIPLO] [🗑]
    //            [strip-icone: ✂️ % 📦 | 📝 €/u]
    //            [qty-stepper] [unità] [× prezzo/u]
    (cart.items||[]).forEach(function(it, idx){
      ...
      // Codice interno a 7 cifre: padStart aggiunge zeri a sinistra se numerico
      var codM7 = it.codM ? (String(it.codM).match(/^\d+$/) ? String(it.codM).padStart(7,'0') : it.codM) : '';
      var codF  = it.codF || '';

      // ── CALCOLO PREZZO TRIPLO ────────────────────────────────────────────
      // pOrig = Prezzo Listino (sarà sbarrato); p = Prezzo Finale (bold arancio)
      var pOrig     = 0;
      var hasDiscount = false;
      if(scagAtt && it._prezzoBase){
        pOrig = parsePriceIT(it._prezzoBase);
        hasDiscount = pOrig > p + 0.005;
      } else if((isSc||isFR) && it._prezzoOriginale){
        pOrig = parsePriceIT(it._prezzoOriginale);
        hasDiscount = pOrig > p + 0.005;
      }
      var subFin      = (p * q).toFixed(2);
      var risparmioTot = hasDiscount ? ((pOrig - p) * q).toFixed(2) : '0.00';

      h += '<div class="pos-cart-card ' + (it._checked ? 'pos-cart-card-checked' : '') + '" id="cart-row-' + idx + '">';

      // ── TOP ROW: [thumb?] [nome+codici] [prezzoTriplo] [🗑] ─────────────
      h += '<div class="pos-card-top">';

      // Thumbnail solo se presente (risparmia spazio orizzontale)
      if(it.foto){
        h += '<div class="pos-card-thumb"><img src="' + it.foto + '" style="width:100%;height:100%;object-fit:cover;border-radius:5px;"></div>';
      }

      // Colonna info: nome + codici
      ...
      if(codM7 || codF){
        ...
        if(codM7 && codF) h += '<span style="color:#3a3a3a;"> · </span>';
        if(codF)  h += '<span style="color:#fc8181;">' + esc(codF) + '</span>';
        h += '</div>';
      }
      h += '</div>';

      // PREZZO TRIPLO: [~~Listino~~] [Finale bold] [🟢 -€risparmio]
      h += '<div class="pos-card-price-row">';
      if(hasDiscount){
        h += '<span class="pos-card-price-listino">€' + (pOrig*q).toFixed(2) + '</span>';
      }
      h += '<span class="pos-card-price-finale">€' + subFin + '</span>';
      if(hasDiscount && parseFloat(risparmioTot) > 0){
        h += '<span class="pos-card-risparmio">-€' + risparmioTot + '</span>';
      }
      h += '</div>';

      // Cestino compatto (28×28px)
      h += '<button class="pos-card-del" onclick="cartRemoveItem(\'' + cart.id + '\',' + idx + ')" title="Rimuovi">🗑</button>';
      h += '</div>'; // fine pos-card-top

      // ── STRIP ICONE SERVIZIO — icone outline ≤18px, orizzontali ────────
      // Sostituisce i vecchi tasti grandi Taglia/Sconto/Scaglioni/Note
      var scOn    = isSc || isFR;
      var hasScag = scagAp || scagAtt;
      h += '<div class="pos-icon-strip">';
      // ✂️ Taglia/Fine Rotolo
      h += '<button class="pos-icon-btn ' + (scOn ? 'active' : '') + '" ';
      h += 'onclick="cartCycleScampolo(\'' + cart.id + '\',' + idx + ')" ';
      h += 'title="' + (isFR ? 'Fine Rotolo' : 'Taglia/Scampolo') + '">✂️</button>';
      // % Sconto/Scaglioni
      h += '<button class="pos-icon-btn ' + (hasScag ? 'active' : '') + '" ';
      h += 'onclick="cartToggleScaglioni(\'' + cart.id + '\',' + idx + ')" ';
      h += 'title="Sconto/Scaglioni" style="font-size:11px;font-weight:800;">%</button>';
      // 📦 Da ordinare/Imballo
      h += '<button class="pos-icon-btn ' + (isDaOrd ? 'active' : '') + '" ';
      h += 'onclick="cartToggleDaOrdinare(\'' + cart.id + '\',' + idx + ')" ';
      h += 'title="Da ordinare">📦</button>';
      // separatore visivo
      h += '<span class="pos-icon-strip-sep"></span>';
      // 📝 Note
      h += '<button class="pos-icon-btn ' + (hasNota ? 'active' : '') + '" ';
      h += 'onclick="cartToggleNotaInline(\'' + cart.id + '\',' + idx + ')" ';
      h += 'title="Nota">📝</button>';
      // Input prezzo unitario inline (compatto, a destra nella strip)
      h += '<div style="display:flex;align-items:center;gap:2px;margin-left:auto;">';
      h += '<span style="font-size:9px;color:var(--pos-muted);">€</span>';
      ...
      h += 'onclick="this.select()" style="width:46px;font-size:11px;padding:2px 4px;" ';
      h += 'title="Prezzo unitario">';
      h += '<span style="font-size:9px;color:var(--pos-muted);">/u</span>';
      h += '</div>';
      h += '</div>'; // fine pos-icon-strip

      // ── Nota preview inline (solo se ha testo) ───────────────────────────
      if(hasNota){
        h += '<div class="pos-card-nota-preview">📝 ' + esc(it.nota) + '</div>';
      }

      // ── RIGA QTÀ: stepper compatto + unità + riepilogo ──────────────────
      ...
      h += '<button class="pos-qty-btn" onclick="cartDelta(\'' + cart.id + '\',' + idx + ',1)">＋</button>';
      h += '</div>';
      h += '<span style="font-size:11px;color:var(--pos-muted);">' + (it.unit||'pz') + '</span>';
      h += '<span style="font-size:11px;color:var(--pos-muted);margin-left:auto;">× €' + esc(it.prezzoUnit||'0') + '</span>';
  ...
  // Tasto 3: CONFERMA E CREA ORDINE (arancione acceso, destra)
  // Invia il carrello alla cassa creando un ordine formale
  if(cart.stato === 'modifica'){
    h += '<button class="pos-big-btn pos-big-btn-confirm" ';
    h += 'onclick="aggiornaOrdine(\'' + cart.id + '\')">';
    h += '✏️<span>AGGIORNA</span></button>';
  } else {
    h += '<button class="pos-big-btn pos-big-btn-confirm" ';
    h += (!(cart.items||[]).length ? 'disabled ' : '');
    h += 'onclick="inviaOrdine(\'' + cart.id + '\')">';
