...
function buildTagsHTML(data,withIdx){
  var _rem=(typeof removed!=='undefined' && removed instanceof Set)?removed:new Set();
  var active=data.filter(function(r,i){return !_rem.has(String(i));});
  var sm=active.filter(function(r){return r.size==='small';});
  var lg=active.filter(function(r){return r.size==='large';});
  var h='';
  function origIdx(r){ return (typeof rows!=='undefined'&&rows)?rows.indexOf(r):-1; }
...
function applyEditorCSS(){
  ...
  if(shape==='pill'||shape==='rounded'){
    promoCSS=
      '.tag-small.cp::before,.tag-large.cp::before{transform:none!important;width:auto!important;padding:0.8mm 2.5mm!important;top:1.5mm!important;right:1.5mm!important;border-radius:3mm!important;letter-spacing:0.8px!important;}'+
      '.tag-small.cp::before{font-size:'+pf+'pt!important;}'+
      '.tag-large.cp::before{font-size:'+(pf+1)+'pt!important;}';
  } else {
    promoCSS=
      '.tag-small.cp::before{font-size:'+pf+'pt!important;width:'+smRibW+'!important;padding:'+smRibPad+' 0!important;top:'+smRibTop+'!important;right:'+smRibRight+'!important;}'+
      '.tag-large.cp::before{font-size:'+(pf+1)+'pt!important;width:'+lgRibW+'!important;padding:'+lgRibPad+' 0!important;top:'+lgRibTop+'!important;right:'+lgRibRight+'!important;}';
  }
  ...
  s.textContent=
    ...
    promoCSS+promoContentCSS+bgCSS+shapeCSS+frameCSS+
    '@media print{'+
...
function ec_setShape(val){
  ...
  document.querySelectorAll('.ec-shape-btn').forEach(function(b){ b.classList.toggle('active',b.dataset.shape===val); });
  lsSet('cp4_editor',editorSettings); applyEditorCSS(); renderEditorPreview(); genTags();
}
function ec_setFrame(val){
  editorSettings.tagFrame=val;
  document.querySelectorAll('.ec-frame-btn').forEach(function(b){ b.classList.toggle('active',b.dataset.frame===val); });
  lsSet('cp4_editor',editorSettings); applyEditorCSS(); renderEditorPreview(); genTags();
}
...
function ec_setPromoText(val){
  ...
  document.querySelectorAll('.ec-promo-txt-btn').forEach(function(b){ b.style.borderColor=b.dataset.val===val?'#e53e3e':''; });
  lsSet('cp4_editor',editorSettings); applyEditorCSS(); renderEditorPreview(); genTags();
}
function ec_setBg(val){
  editorSettings.tagBg=val||'#ffffff';
  document.querySelectorAll('.ec-bg-btn').forEach(function(b){ b.style.borderColor=b.dataset.val===val?'var(--accent)':'var(--border)'; });
  lsSet('cp4_editor',editorSettings); applyEditorCSS(); renderEditorPreview(); genTags();
}
