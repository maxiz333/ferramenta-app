...
function renderTable(){
  ...
  if(!tb) return; // elemento non ancora nel DOM, skip
  tb.innerHTML='';
...
function delRow(i){
  ...
  removed=newRem; lsSet(RK,Array.from(removed));
...
function toggleRem(i,cb){
  ...
  lsSet(RK,Array.from(removed));updateStats();
}
function clearChecked(){
  showConfirm('Rimuovere dalla tabella tutti gli articoli spuntati?', function(){

  var toRemove=Array.from(removed).map(Number).sort(function(a,b){return b-a;});
...
function onDrop(e,sezione){
  ...
  if(sezione==='magazzino'){
    var dz=document.getElementById('drop-zone-mag');if(dz)dz.classList.remove('over');
  } else {
    var dz=document.getElementById('drop-zone');if(dz)dz.classList.remove('over');
    var dzT1=document.getElementById('drop-zone-t1');if(dzT1)dzT1.classList.remove('over');
  }
...
function applyEditor(){
  ...
  editorSettings.smW=parseInt(document.getElementById('ec-sm-w').value,10)||67;
  editorSettings.smH=parseInt(document.getElementById('ec-sm-h').value,10)||38;
  editorSettings.lgW=parseInt(document.getElementById('ec-lg-w').value,10)||94;
  editorSettings.lgH=parseInt(document.getElementById('ec-lg-h').value,10)||39;
